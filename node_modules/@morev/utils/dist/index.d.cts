...
type Result<Source extends readonly any[], Type, SampleSize> = Source['length'] extends 0 ? undefined : SampleSize extends 1 ? NeverAsUndefined<Type, false> : NeverAsUndefined<Type, true>;
/**
 ...
 */
declare const arraySample: <const T, Size extends number = 1>(array: readonly T[], size?: Size, oversize?: boolean) => Result<typeof array, T, Size>;
...
type ToReturn$7<T> = T extends any[] ? T : T[];
/**
 ...
 */
declare const toArray: <T>(value: T) => ToReturn$7<T>;
 ...
 */
declare const contrastColor: (sourceColor: string, userCandidates?: string[] | null, userOptions?: Partial<Options$g>) => string | null;

type AlphaChannelOption = {
    /**
 ...
 */
type ObjectLongAlpha = ObjectLong & {
    /**
 ...
 */
type ObjectLongMaybeAlpha = ObjectLong & {
    /**
     * Value of opacity channel in range `[0..1]`,
 ...
 */
type ObjectShortAlpha = ObjectShort & {
    /**
 ...
 */
type ObjectShortMaybeAlpha = ObjectShort & {
    /**
 ...
 */
type RgbArrayAlpha = [number, number, number, number];
/**
 * A tuple of form `[red, green, blue, alpha]`. \
 * First three values are represented as numbers in range `[0..255]`.
 * Fourth element represented as number in range `[0..1]` if the original HEX provides information about alpha channel.
 */
type RgbArrayMaybeAlpha = [number, number, number] | [number, number, number, number];
...
type AlphaShorthand<AlphaChannel extends Options$f['alphaChannel'] | undefined, NeverValue, AlwaysValue, IfPresentedValue> = AlphaChannel extends 'never' ? NeverValue : AlphaChannel extends 'if-presented' ? IfPresentedValue : AlphaChannel extends 'always' ? AlwaysValue : AlwaysValue;
type AlphaNotationType<Notation extends Options$f['cssAlphaNotation'] | undefined, PercentageValue, NumberValue> = Notation extends 'number' ? NumberValue : PercentageValue;
type ToReturn$6<UserOptions extends Partial<Options$f>> = UserOptions['returnType'] extends 'css-legacy' ? UserOptions['alphaChannel'] extends 'never' ? CssLegacy : AlphaNotationType<UserOptions['cssAlphaNotation'], CssLegacyAlphaPercentage, CssLegacyAlphaNumber> : UserOptions['returnType'] extends 'css-modern' ? UserOptions['alphaChannel'] extends 'never' ? CssModern : AlphaNotationType<UserOptions['cssAlphaNotation'], CssModernAlphaPercentage, CssModernAlphaNumber> : UserOptions['returnType'] extends 'object-long' ? AlphaShorthand<UserOptions['alphaChannel'], ObjectLong, ObjectLongAlpha, ObjectLongMaybeAlpha> : UserOptions['returnType'] extends 'array' ? AlphaShorthand<UserOptions['alphaChannel'], RgbArray, RgbArrayAlpha, RgbArrayMaybeAlpha> : AlphaShorthand<UserOptions['alphaChannel'], ObjectShort, ObjectShortAlpha, ObjectShortMaybeAlpha>;
 ...
 */
declare const hexToRgb: <UserOptions extends Partial<Options$f> = Options$f>(hex: string, userOptions?: UserOptions) => ToReturn$6<UserOptions> | null;

type Options$e = AlphaChannelOption & {
    /**
 ...
 */
declare const normalizeHex: (hex: string, userOptions?: Partial<Options$e>) => string | null;

type Input$2 = string | RgbArrayMaybeAlpha | ObjectLongMaybeAlpha | ObjectShortMaybeAlpha;
type Options$d = AlphaChannelOption & {
    /**
     * Preferable HEX notation.
     *
     * Examples of the short form: `#000`, `#000f`.
 ...
 */
declare const rgbToHex: (source: Input$2, userOptions?: Partial<Options$d>) => string | null;
...
declare const tsObject: {
     ...
     */
    keys: <T extends PlainObject>(obj: T) => Array<ObjectKeys<Required<T>>>;
    /**
     ...
     */
    values: <T extends PlainObject>(obj: T) => ObjectValues<Required<T>>;
    /**
     ...
     */
    entries: <T extends PlainObject>(obj: T) => ObjectEntries<Required<T>>;
    /**
     ...
     */
    fromEntries: <Key extends PropertyKey, Entries extends ReadonlyArray<readonly [Key, any]>>(entries: Entries) => ObjectFromEntries<Key, Entries>;
};
 ...
 */
declare const minTimePromise: <T>(promise: Promise<T>, minTime?: number) => Promise<unknown>;
