...
var arraySample = (array, size = 1, oversize = false) => {
  if (!array?.length || !isInteger(size) || size < 1) return void 0;
  if (size === 1) {
...
var hexToRgb = (hex, userOptions) => {
  ...
  if (options.returnType === "object-short") {
    const base = { r, g, b };
    if (options.alphaChannel === "never") return base;
...
var formDataToObject = (formData) => {
  if (!isFormData(formData)) return formData;
  const result = {};
...
var objectKeysCase = (input, neededCase, _options2) => {
  ...
  const processEntry = (entry, path, depth) => {
    ...
    return Object.entries(entry).reduce((acc, [_key, _value]) => {
      ...
      if (isEntryArray) acc.push(value);
...
var serialize = (key, value, options, formData) => {
  ...
  if (isArray(value)) {
    ...
    value.forEach((arrayValue, index) => {
      const arrayKey = (() => {
        if (options.arrayBrackets === "append" || options.arrayBrackets === "append-only-for-files" && isFilesArray) return `${key}[${options.indices ? index : ""}]`;
...
var FormatBytes = class {
  ...
  constructor(bytes, customOptions) {
    ...
    const options = mergeObjects(DEFAULT_OPTIONS6, customOptions);
...
var rgbToHex = (source, userOptions) => {
  const options = { ...DEFAULT_OPTIONS7, ...userOptions };
  const toHex = (r, g, b, a) => {
...
var parseResponseCookie = (input, userOptions) => {
  const options = { ...DEFAULT_OPTIONS8, ...userOptions };
  if (isEmpty(input)) {
    return options.returnType === "array" ? [] : {};
  }
  const setCookieValue = toArray((() => {
    if (isString(input)) return splitCookiesString(input);
    if (isArray(input)) return input.reduce((acc, value) => [...acc, ...splitCookiesString(value)], []);
...
var serializeCookie = (cookie, userOptions) => {
  const options = { ...DEFAULT_OPTIONS9, ...userOptions };
...
var DEFAULT_OPTIONS10 = { utc: false };
var parseDate = (input, userOptions) => {
  const options = { ...DEFAULT_OPTIONS10, ...userOptions };
  if (isDate(input)) {
...
var DEFAULT_OPTIONS11 = { utc: false };
var toDate = (value, userOptions) => {
  const options = { ...DEFAULT_OPTIONS11, ...userOptions };
...
var randomDate = (start = /* @__PURE__ */ new Date(0), end = /* @__PURE__ */ new Date(), userOptions) => {
  if (!isDate(start, true)) start = /* @__PURE__ */ new Date(0);
  if (!isDate(end, true)) end = /* @__PURE__ */ new Date();
...
var getWindowScroll = (axis = "y") => {
  if (axis === "y") return getScrollTop();
  if (axis === "x") return getScrollLeft();
...
var getElementOffset = (el, axis = "both", parent = window) => {
  ...
  if (axis === "y") return top + scroll;
  if (axis === "x") return left + scroll;
  return {
...
var compose = (...functions) => (value) => [...functions].reverse().reduce((acc, curr) => {
  acc = curr(acc);
  return acc;
}, value);
